...
import { guestApi, paymentApi, roomApi } from '../api/entities';
import { Booking, db, getLastSyncTimestamp, Guest, Payment, Room, setLastSyncTimestamp } from '../db/dexie';

const ENTITIES = ['bookings', 'rooms', 'guests', 'payments'] as const;
  ...
  private static async pushLocalChanges() {
    ...
    for (const item of queue) {
      try {
        ...
        switch (item.entity) {
          case 'bookings':
            if (item.action === 'create') response = await bookingApi.create(item.data);
            if (item.action === 'update') response = await bookingApi.update(item.entityId, item.data);
            break;
          // Add other entities...
        }
